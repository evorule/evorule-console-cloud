...
- **deploy-gitee.yml 流水线删除（Gitee Pages 停服终审，2026-09-15）** — Gitee Pages 平台服务已停服（官方答复「该功能已下线」；管理入口 / 站点域名 / 帮助文档均 404，四条独立证据见 74 号文档《双平台部署与离线缓存配置说明》§6），该流水线永远无法成功，仅持续产生红色流水线噪音。README 中失效的 Gitee Pages 镜像启用指引同批清理（在线入口只保留 GitHub Pages）；历史版本条目保留原文并加停服标注。国内访问替代方案按 74 号文档 §6.5 另行承接
...
- **AI 草案校验错误通俗化** — 助手「创建规则」对话框(draft dialog)的校验错误现统一经 `friendlyRuleError` 翻译层转成人话,不再直接展示 "exists.path 必填" 等底层技术术语(与业务表单/建库向导一致)
- **试运行版本号展示收口** — 建库向导试运行结果在拿不到实际版本号时不再补零展示 `version=0`,改为仅在确实读到版本时附加显示,消除误导性版本信息

## [0.4.3] - 2026-09-10

**发布概念三层收敛 + 向导/路由打磨** — "发布"语义按治理/执行/工作区三层定名,消除"发布意味着什么"的歧义

### 🆕 新增

- **发布概念三层收敛** — 治理中心生命周期状态统一为「上架/编目」(未生效),发布队列更名为「部署审批」(执行生效、版本递增),工作区规则状态统一为「可用」;UI 文案与状态词全面对齐新语义,避免"发布"一词三层混用的歧义
- **部署审批入口更名** — 导航与相关入口统一改用「部署审批」,消费者可见文案不再出现"发布队列"字样
...
- **应用管理页** — 平台管理新增应用凭据管理入口(/apps,manage_apps 权限门控):外部应用 API Key 的签发/列表/吊销一站操作;key 明文仅签发成功弹窗一次性展示并支持复制,关闭后不可再取(服务端只存哈希);吊销二次确认并提示即时生效语义;角色编辑器权限矩阵同步新增"应用凭据管理"权限点;导航/命令面板入口按 manage_apps 权限自动显隐
- **应用配额管理** — 应用管理页支持配额可视与调整:签发表单可设置限速(次/秒)与日配额(留空=不限,非法输入如实提示不提交);列表新增限速与今日用量列,用量达日配额醒目标红;配额调整弹窗预填当前值,全量覆盖语义(两项均下发,清空即不限),已用量保留不清零、调整即时生效
- **插件审批工作台** — 新增插件审批视图:集中呈现外部插件及其运行时探活状态(在线/离线徽标,与平台健康端点一致),按插件分组展示待审配置提案,批准/拒绝一站操作;审批请求统一经 server 审批代理执行,审批人身份由服务端从平台登录态强制注入,前端不自报、不可伪造;未连接平台时提供连接引导,插件离线/不可达时与空提案列表如实区分
...
- **本地草稿语义澄清** — 建库向导步骤4/步骤5与工作台总览统一用「本地草稿 vs 正式发布」表述(替代"浏览器本地/执行域/治理链"术语堆叠);总览与诊断面板的「规则集版本」改名「正式规则集版本」,未发布时显示「未发布」并附 tooltip 说明
- **LLM 配置指引** — 建库向导 LLM 辅助按钮未配置禁用时,新增引导条指向「右侧💬配置LLM」与「设置→LLM配置」入口
- **首访引导去重** — 工作台新手横幅可见时不再叠加 GuidedHint 与推荐卡;建库完成 toast 由父组件统一弹出,消除双 toast 叠加
- **登录页文案通俗化** — 平台账号/演示模式说明去掉「服务端实时下发/平台管理面/业务功能面」等技术术语
- **执行台业务化** — 默认指令示例改为业务语义(报销金额阈值),指令编辑区新增通俗说明;「应用规则」按钮优先显示中文描述;「Payload 注入」改名「修改业务数据(Payload)」并软化提示;工作台一键操作默认规则 JSON 同步业务化
- **JSON 树展开提示** — 执行结果树根节点显示「点击展开/收起」动态提示,折叠态不再无任何线索
- **审计/导出空态提示** — 无记录时验证/导出/导入提示改为「请先在执行台提交一条命令产生记录」,内联空态提示同步通俗化
- **治理中心密码占位** — 密码输入框占位符由圆点改为「请输入密码」,避免误以为已填充
- **五态生命周期补全** — 治理中心首访提示补上 Rejected,五态完整列出
- **开发者模式 JSON 编辑器** — 规则库开发者模式落地可用的 JSON 编辑视图(规则列表/选中懒加载/编辑/校验/保存,draft 限定、内置规则只读提示),替代原「正在适配」占位
- **无业务元数据提示** — 规则卡片无业务场景时提示「通用规则·未关联业务场景」并给出关联入口,替代晦涩的「无业务元数据(开发者模式可编辑 JSON)」
- **启动脚本 --insecure-serve** — start-all.ps1 服务端启动参数补 `--insecure-serve` 显式声明,适配 v0.5.0 loopback 无认证启动策略(官方分发包 start-evorule.bat 已由 server 仓修复)

## [0.4.1] - 2026-09-06

**未登录访问引导修复** — 受登录守卫的页面不再静默弹回首页，统一引导至登录页

### 🐛 修复

- **未登录访问引导** — 未登录直接访问市场/运行时/工作区/导出/导入导出/视图页时，统一 307 引导至登录页(此前静默弹回首页,项目方易误判"页面损坏");登录后各页面可达性不变
- **市场导航锁态标注** — 市场导航项补充登录要求标注，未登录场景的锁态提示与其他受守卫入口口径一致

## [0.4.0] - 2026-09-06

**官方资产市场** — 市场新增官方资产分区：治理中心公开数据集一站发现 / 检视 / 部署导航

### 🆕 新增

- **官方资产分区** — 模板市场新增「官方资产」分区:自动发现治理服务中公开且已发布的数据集(版本/标签/发布状态徽章);未连接时提供连接引导与已存凭据一键重连
- **官方资产预览包下载** — 一键导出数据集预览包(JSON,含版本与条目清单);预览包未附部署证据,仅作检视用途,导入执行域会被证据闸门拒绝(语义如实标注)
- **官方资产部署导航** — 从市场一键跳转治理中心既有部署面板(证据声明/dry-run 预检/部署确认),不绕过任何证据闸门

### 🔄 变更

- **开发模式治理连接免 CORS 配置** — dev 下治理服务连接默认经开发代理转发至本机治理端口(同源请求),无需为开发端口调整治理服务的允许来源白名单;生产构建行为不变(直连治理服务)

## [0.3.0] - 2026-09-06

**治理能力深化与知识数据面** — 测试工作台 / 知识条目在线编辑 / 权限管理 UI / 模板市场 server 接线 / PDF 服务端渲染 / 规则编辑深化 / 旅程可用性修复
...
> 规划中：本地 LLM（L2）付费扩展 + 依社区反馈优化迭代。

---

## [0.1.0] - 2026-08-26

> 首发占位版：联网大众版（内核 + 联网 + 云 LLM）。开发期（Phase 1-7）完成，自动化测试全绿（svelte-check 0/0 + vitest 904/904 + playwright 64/64），发版治理文件齐全，项目方人工验收通过。
...
- 加载状态 + 错误提示 + 重试机制
- Escape 键关闭 Dialog
- **项目方审核确认**：LLM 草案/输入必须项目方点“采用”才生效（不自动执行）
...
2. AI 按钮仅在项目方显式点击 + Dialog 确认后才调用 LLM
3. LLM 调用失败时降级为“项目方手动编辑 JSON”，不阻塞规则引擎工作
...
发版前需项目方人工测试以下场景（自动化测试覆盖之外的 UX / 真实环境）：
