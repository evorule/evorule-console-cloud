...
### 🐛 修复（首次用户体验缺陷 UV-151~UV-165 集中整治）

- **UV-151 校验提示通俗化** — 规则校验错误新增通俗化翻译层(ruleValidator.friendlyRuleError):业务表单/建库向导不再展示 "exists.path 必填" 等技术术语,改用人话提示并保留未命中消息原样透出
- **UV-152 试运行版本号显示** — 建库向导试运行结果不再显示 "version=?" ,提交后从 reactor 版本取实际版本号
- **UV-153 本地草稿语义澄清** — 建库向导步骤4/步骤5与工作台总览统一用「本地草稿 vs 正式发布」表述(替代"浏览器本地/执行域/治理链"术语堆叠);总览与诊断面板的「规则集版本」改名「正式规则集版本」,未发布时显示「未发布」并附 tooltip 说明
- **UV-154 LLM 配置指引** — 建库向导 LLM 辅助按钮未配置禁用时,新增引导条指向「右侧💬配置LLM」与「设置→LLM配置」入口
- **UV-155 首访引导去重** — 工作台新手横幅可见时不再叠加 GuidedHint 与推荐卡;建库完成 toast 由父组件统一弹出,消除双 toast 叠加
- **UV-156 登录页文案通俗化** — 平台账号/演示模式说明去掉「服务端实时下发/平台管理面/业务功能面」等技术术语
- **UV-157 执行台业务化** — 默认指令示例改为业务语义(报销金额阈值),指令编辑区新增通俗说明;「应用规则」按钮优先显示中文描述;「Payload 注入」改名「修改业务数据(Payload)」并软化提示;工作台一键操作默认规则 JSON 同步业务化
- **UV-158 JSON 树展开提示** — 执行结果树根节点显示「点击展开/收起」动态提示,折叠态不再无任何线索
- **UV-159 审计/导出空态提示** — 无记录时验证/导出/导入提示改为「请先在执行台提交一条命令产生记录」,内联空态提示同步通俗化
- **UV-162 治理中心密码占位** — 密码输入框占位符由圆点改为「请输入密码」,避免误以为已填充
- **UV-163 五态生命周期补全** — 治理中心首访提示补上 Rejected,五态完整列出
- **UV-164 开发者模式 JSON 编辑器** — 规则库开发者模式落地可用的 JSON 编辑视图(规则列表/选中懒加载/编辑/校验/保存,draft 限定、内置规则只读提示),替代原「正在适配」占位
- **UV-165 无业务元数据提示** — 规则卡片无业务场景时提示「通用规则·未关联业务场景」并给出关联入口,替代晦涩的「无业务元数据(开发者模式可编辑 JSON)」
- **启动脚本 --insecure-serve(UV-150 同类)** — start-all.ps1 服务端启动参数补 `--insecure-serve` 显式声明,适配 v0.5.0 loopback 无认证启动策略(官方分发包 start-evorule.bat 已由 server 仓修复)
