...
export type PermissionAction =
	...
	// 平台管理 4 点(,server 权限点注册表权威定义)
	| 'manage_users' // 用户管理(增删改/启停/改角色)
	| 'manage_roles' // 角色管理(自定义角色 + 权限集)
	| 'view_users' // 查看用户列表(只读)
	| 'manage_apps'; // 应用凭据管理(签发/列表/吊销外部应用 API Key)

/** 平台管理 4 点(角色矩阵编辑器分组渲染用;demo 矩阵不含) */
