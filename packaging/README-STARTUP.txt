...
------------------------------
先看 data\logs\ 下的日志文件:
- evorule-server.log       主服务运行日志(启动失败时看其中
                           ERROR 行,含 netstat 定位占用进程的
                           自诊断指引)
- evorule-server-stderr.log 主服务启动期错误输出(端口被占用等
                           启动失败会写到这里)
- evorule-rule.log         治理服务日志(stdout+stderr 合并)
端口被占用是最常见原因(上次未退出的残留实例):
在命令行执行 netstat -ano | findstr :18080 找到 PID,
任务管理器结束该进程后重新启动。
