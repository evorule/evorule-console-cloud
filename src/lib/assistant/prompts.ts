...
// evorule-console-cloud — LLM 用途 prompt 模板(规则草案/解释/测试输入/命令转译/流程转译)
//
// 设计原则:
//   - 强约束 LLM 输出纯 JSON(规则草案 / 测试输入 / 命令草稿 / flow 草稿),便于机器解析
 ...
 * (collect/merge 已退役规则清理 2026-09-14;enforce 仅 L2 层级文件[00_constraint_ 新前缀/00_meta_ 旧兼容 + L2 tier 声明],业务文件不出现)
 */
...
// ============================================================================
// 用途6: NL → flow JSON 草稿(流程画布转译器)
// ============================================================================

/**
 * 用途6: 自然语言 → flow JSON 草稿。
 *
 * 镜像自 evorule-console src/lib/assistant/prompts.ts 的 few-shot 调优版
 * (console 为公开仓 SSOT,mock 测试锁定;两仓无 import 通道,镜像复制是
 * kernel 区既定模式)。变更须双仓同步。
 *
 * 设计:
 *   - ctx(nodeTypes/sceneFields)由画布页从已加载 pack 资产投影(R4:语义
 *     全来自资产声明,零领域硬编码)
 *   - flow schema 协议标识符原样引用不翻译(R5)
 *   - few-shot 结构示例仅演示 JSON 形状,显式禁止照抄(防 LLM 搬运虚构
 *     node_type);空资产如实呈现
 *   - 产物是**草稿**:填入画布由用户可见可改,人点击编译才走既有
 *     compileFlow→Draft→Publish 链(R3)——LLM 永不直接 compile/publish
 */
